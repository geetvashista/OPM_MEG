...
def get_mean_mne_psd(file_path):
    raw_data = np.load(file_path)
    raw_data = raw_data.T
    ...
    # picks= [i for i in range(4)]
    fig = data.compute_psd(fmin=1, fmax=30, average='mean')
    fig.plot(average=True)

    return fig


def plot_mne(participant, run):
    # Get the data we need
    fig = get_mean_mne_psd(
        r'C:\Users\em17531\Desktop\OPM_MEG\data\run_' + str(run) + '\sub-00' + str(participant) + '_run-00' + str(
            run) + '_VE_1_40_Hz_Z.mat_array.npy')


# __Main__

plot_mne(participant=1, run=2)
