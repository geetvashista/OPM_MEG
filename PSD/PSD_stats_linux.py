...
def get_mean_psd_data(participant, run):
    if participant <= 9.:
        file_path = r'/home/sahib/Documents/OPM_MEG/data/' \
                    '/run_' + str(run) + '/sub-00' + str(participant) + \
        ...
        file_path = r'/home/sahib/Documents/OPM_MEG/data/' \
                    '/run_' + str(run) + '/sub-0' + str(participant) + \
                    '_run-00' + str(run) + '_VE_1_40_Hz_Z.mat_array.npy'
...
stat_output = stats.ttest_rel(psd_run_1.mean(axis=-1), psd_run_2.mean(axis=-1))
...
names_for_df = []
for i in sig_index:
    names_for_df.append(ROI_names[i])
    print(ROI_names[i])
